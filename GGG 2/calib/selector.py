...
class CalibratorSelector:
    ...
    def fit_pick(self, raw_scores, y, input_is_logit=False, weights=None) -> Tuple[str, Any, Dict[str, float]]:
        ...
        metrics = {}
        best_name, best_obj, best_val = None, None, +1e18
        for name, Cal in self._candidates(len(y)).items():
            obj = Cal.fit(raw_scores, y, sample_weight=weights) if name!="temp" else Cal.fit(raw_scores, y)
            p = obj.predict_proba(raw_scores, input_is_logit=input_is_logit) if hasattr(obj, "predict_proba") else obj.predict_proba(raw_scores)
            cur = nll(y, p) if self.pick_by=="nll" else brier(y, p)
            metrics[name] = float(cur)
            if cur < best_val:
                best_val, best_name, best_obj = cur, name, obj
        metrics["ece"] = float(ece(y, best_obj.predict_proba(raw_scores if not input_is_logit else raw_scores, input_is_logit=input_is_logit)))
