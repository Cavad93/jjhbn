...
class MetaStacking:
    ...
    def _maybe_flip_modes(self):
        """
        Переключение режимов SHADOW ↔ ACTIVE на основе качества калибровки мета-модели.
        
        Логика:
        - В режиме SHADOW мета делает прогнозы, но они не используются для реальных ставок
        - Используется базовая вероятность (более стабильная и консервативная)
        - При улучшении калибровки → переход в ACTIVE (мета используется в ставках)
        - При ухудшении калибровки → возврат в SHADOW (защита капитала)
        
        Преимущества над блокировкой торговли:
        - Бот продолжает торговать и собирать данные для обучения
        - Мета-модель продолжает учиться в фоне (SHADOW режим)
        - Капитал защищён использованием базовой модели при плохой калибровке
        - Автоматическое возобновление использования меты при улучшении качества
        """
        
        # Вспомогательная функция для расчёта винрейта (оставляем для логирования)
        def wr(arr, n):
            if len(arr) < n: return None
            window = arr[-n:]
            return 100.0 * (sum(window)/len(window))
        
        # Минимальное количество наблюдений для принятия решения о переключении
        min_observations = max(50, self.cfg.min_ready // 2)
        
        # === SHADOW → ACTIVE: проверяем калибровку в теневом режиме ===
        if self.mode == "SHADOW":
            # Нужно достаточно наблюдений для надёжной оценки калибровки
            if len(self.shadow_hits) < min_observations:
                return  # Ещё рано принимать решение
            
            # Вычисляем качество калибровки на теневых прогнозах
            # Используем последние N прогнозов для оценки текущего состояния
            try:
                # Берём последние прогнозы и их исходы из shadow_hits
                # shadow_hits содержит 1 (правильный прогноз) или 0 (неправильный)
                window_size = min(200, len(self.shadow_hits))
                
                # Для оценки калибровки нам нужны сами вероятности, а не только хиты
                # Поэтому читаем данные из CSV, где хранятся p_meta_raw для теневых сделок
                calib_err = rolling_calib_error(CSV_SHADOW_PATH, n=window_size)
                
                if calib_err is None:
                    # Не удалось вычислить - остаёмся в текущем режиме
                    return
                
                # Пороги качества калибровки (ECE - Expected Calibration Error)
                CALIB_EXCELLENT = 0.03  # ≤ 3% - отличная калибровка
                CALIB_GOOD = 0.05       # ≤ 5% - хорошая калибровка
                CALIB_ACCEPTABLE = 0.07 # ≤ 7% - приемлемая калибровка
                
                # Переходим в ACTIVE если калибровка приемлемая или лучше
                if calib_err <= CALIB_ACCEPTABLE:
                    self.mode = "ACTIVE"
                    if HAVE_RIVER:
                        self.adwin = ADWIN(delta=self.cfg.adwin_delta)
                    
                    # Для логирования также покажем винрейт
                    ...
                    print(f"[meta] SHADOW→ACTIVE: калибровка {calib_quality} (ECE={calib_err:.3f}), "
                        f"shadow_wr={wr_shadow:.1f}% (n={len(self.shadow_hits)})")
                    
                    try:
                        tg_send(f"🟢 <b>Мета: SHADOW→ACTIVE</b>\n"
                            f"Калибровка: <b>{calib_quality}</b> (ECE={calib_err:.3f})\n"
                            f"Shadow винрейт: {wr_shadow:.1f}% (n={len(self.shadow_hits)})\n"
                            f"Мета-модель активирована для реальных ставок")
                    except:
                        pass
                    
            except Exception as e:
                print(f"[meta] ошибка проверки калибровки для SHADOW→ACTIVE: {e}")
                return
        
        # === ACTIVE → SHADOW: проверяем деградацию калибровки в активном режиме ===
        elif self.mode == "ACTIVE":
            # В активном режиме проверяем чаще (меньше наблюдений нужно для детекции проблемы)
            active_min_obs = max(30, self.cfg.min_ready // 3)
            
            if len(self.active_hits) < active_min_obs:
                return  # Ещё рано проверять
            
            try:
                # Проверяем калибровку на активных сделках (из основного CSV)
                ...
                # Также проверяем ADWIN на дрейф (если доступен)
                adwin_drift_detected = False
                if self.adwin is not None and len(self.active_hits) > 0:
                    try:
                        # ADWIN уже обновляется в record_result, здесь только читаем состояние
                        # Если был дрейф, ADWIN уже перевёл mode в SHADOW
                        if self.mode == "SHADOW":  # ADWIN уже переключил
                            adwin_drift_detected = True
                    except:
                        pass
                
                # Порог деградации калибровки - более строгий чем для входа
                CALIB_DEGRADED = 0.08  # > 8% - калибровка деградировала, защита капитала
                
                # Возвращаемся в SHADOW если калибровка стала плохой ИЛИ ADWIN детектировал дрейф
                if calib_err > CALIB_DEGRADED or adwin_drift_detected:
                    self.mode = "SHADOW"
                    self.active_hits = []  # Сбрасываем историю для чистого старта
                    ...
                    print(f"[meta] ACTIVE→SHADOW: {reason}, "
                        f"active_wr={wr_active:.1f}% (n={len(self.active_hits)})")
                    
                    try:
                        tg_send(f"🟡 <b>Мета: ACTIVE→SHADOW</b>\n"
                            f"Причина: {reason}\n"
                            f"Active винрейт: {wr_active:.1f}% (n={len(self.active_hits)})\n"
                            f"Переход на базовую модель для защиты капитала\n"
                            f"Мета продолжит обучение в фоне")
                    except:
                        pass
                    
            except Exception as e:
                print(f"[meta] ошибка проверки калибровки для ACTIVE→SHADOW: {e}")
                return
