...
# ========== УЛУЧШЕННАЯ ОБРАБОТКА RPC TIMEOUT С КЭШИРОВАНИЕМ ==========
                ...
                if epoch not in bets:
                    ...
                    if epoch == cur and now < rd.lock_ts:
                        ...
                        # ========== СНАЧАЛА СОЗДАЕМ MARKET_FEATURES ==========
                        # --- NEW: режим для контекстного гейтинга ---
                        ...
                        try:
                            ...
                            market_features = market_calc.calculate_features()  # ← market_features создана
                            
                        except Exception as e:
                            print(f"[ERROR] MarketFeaturesCalculator update failed: {e}")
                            market_features = market_calc._default_features()

                        # ========== ТЕПЕРЬ МОЖНО ИСПОЛЬЗОВАТЬ MARKET_FEATURES ==========
                        # ========== НОВОЕ: РАСШИРЕННЫЕ ФИЧИ ДЛЯ ЭКСПЕРТОВ ==========
                        additional_features = []
                        ...
                        try:
                            ...
                            if len(kl_df) >= 5:
                                ...
                                if len(rsi_series) >= 5:
                                    additional_features.append(float(rsi_series.iloc[-2]))
                                    additional_features.append(float(rsi_series.iloc[-5]))
                                else:
                            ...
                            if len(kl_df) >= 15:
                                ...
                                additional_features.append((close_now - close_5m) / max(1e-12, close_5m))
                                additional_features.append((close_now - close_15m) / max(1e-12, close_15m))
                            else:
                            ...
                            if len(kl_df) >= 20:
                                ...
                                additional_features.append(float(normalized_position))
                            else:
                            ...
                            if len(kl_df) >= 15:
                                ...
                                if len(rsi_series) >= 15:
                                    ...
                                    additional_features.append(rsi_vol / 50.0)
                                else:
                                    additional_features.append(0.0)
                            else:
                                additional_features.append(0.0)
                            
                            # 6. ВОЛАТИЛЬНОСТЬ ЦЕНЫ
                            if len(kl_df) >= 20:
                                ...
                                additional_features.append(float(np.clip(cv, 0.0, 5.0)))
                            else:
                                additional_features.append(0.0)
                            
                            # 7. КОНТЕКСТНЫЕ ФИЧИ ИЗ MARKET_FEATURES
                            # ✅ Теперь market_features уже существует!
